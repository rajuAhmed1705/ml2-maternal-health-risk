...
- **γ (Gamma):** Defines the influence of a single training example. A low gamma makes the influence of each training point large, while a high gamma makes it small.
...
For Random Forest, we set `ntree = 500` and tuned `mtry` over values 2, 3, 4, and 5 to find the best number of features considered at each split. For SVM, we tuned `C` (cost) over 0.1, 1, and 10, and `sigma` over 0.01, 0.1, and 1 to control the margin flexibility and RBF kernel width.
